...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    // Step 1: Build header string: "blob 16\0", "tree 16\0", etc.
    const char *type_str;
    ...
    // +1 to include the null byte in the header
    ...
    // Step 4: Deduplication — if object already exists, skip
    ...
    // Step 6: Build final path and a temp path
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // Step 5: Return data portion (after the '\0')
    size_t header_len = null_pos - buf + 1;
    *len_out = total - header_len;
